...
class ItemInstance:
    ...
    fuel_remaining: Optional[int] = None  # Current fuel for light sources (starts at light_duration)
    ...
    def get_display_name(self, data, player_level: int = 1, detect_magic: bool = False) -> str:
        """
        ...
        is_type_identified = data.is_item_type_identified(self.item_id)
        
        if needs_identification and not self.identified and not is_type_identified:
            unknown_name = data.get_unknown_name(self.item_id)
    ...
    def identify(self, data):
        ...
        self.tried = False
        data.identify_item_type(self.item_id)
    ...
    def from_dict(cls, data: Dict) -> ItemInstance:
        ...
        if (
            ...
            instance.slot = "light"
            # Ensure fuel_remaining is set for loaded light sources
            if instance.fuel_remaining is None:
                instance.fuel_remaining = instance.light_duration
        return instance
    
    @classmethod
    def from_template(cls, item_id: str, item_data: Dict) -> ItemInstance:
        """
        Create an item instance from a template in the item data files.
        
        Args:
            item_id: The item template ID
            item_data: The item definition from the loader
        ...
        if isinstance(effect, list) and len(effect) >= 3 and effect[0] == "light_source":
            ...
            instance.fuel_remaining = int(effect[2])  # Initialize with full fuel
            instance.slot = "light"
        return instance

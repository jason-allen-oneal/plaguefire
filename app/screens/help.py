...
TILE_LEGEND: List[Tuple[str, List[Tuple[str, str]]]] = [
    ...
    (
        ...
        [
            ...
            (f"[dim]{FLOOR}[/dim]", "Open floor / explored space"),
            ("[dim]#[/dim] (secret)", "Hidden door disguised as a wall"),
            (f"[dim]{MAGMA_VEIN}[/dim]", "Lava or molten rock"),
            (f"[dim]{QUARTZ_VEIN}[/dim]", "Mineral or quartz vein (mineable)"),
            (f"[bright_white]{STAIRS_UP}[/bright_white]", "Stairs up"),
            (f"[bright_white]{STAIRS_DOWN}[/bright_white]", "Stairs down"),
        ],
    ),
    (
        "Doors & Passageways",
